...
class DividerItemDecoration(context: Context, @DimenRes insetRes: Int) : ItemDecoration() {
    private val mDivider: Drawable? = context.obtainStyledAttributes(intArrayOf(android.R.attr.listDivider)).use {
            it.getDrawable(0)
        }
    private val inset: Int = context.resources.getDimensionPixelSize(insetRes)
    private val size = mDivider?.intrinsicHeight ?: 0
        ...
        parent.layoutManager?.let { layoutManager ->
            ...
            loop@ for (i in 0 until childCount) {
                ...
                when (layoutManager.getItemViewType(child)) {
                    R.id.viewtype_item -> if (i == childCount - 1 || i == childCount - 2 && layoutManager.getItemViewType(
                            parent.getChildAt(i + 1)
                        ) != R.id.viewtype_item
                    ) dividerRect.left = paddingLeft else dividerRect.left = paddingLeftInset
